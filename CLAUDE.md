...
- Recurring series keep their weekday/rhythm: the next occurrence is computed from `cycleDueDate || dueDate`. An approved postpone stores the original scheduled date in `cycleDueDate` (first postpone wins) and only moves that one occurrence; overdue completion also counts from the scheduled date, never from the completion date. Changing the date in the edit form clears `cycleDueDate` — that is how the schedule itself is changed.
- A task can never be dated before the day it was created: the form defaults to `max(selectedDate, todayIso())`, blocks past dates (an overdue task may still be saved with its unchanged date), `selectedDate` is reset to today when the app is resumed on a new day, and `naprawTerminyZPrzeszlosci()` moves any open task whose due date precedes its "Utworzono zadanie" entry onto its creation day.
- Open standard (non-shopping, non-reward) tasks are always worth `PRIORITY[priority].points`: the edit form recomputes it and `normalizeState` re-derives it on load. Don't route that through `getTaskPotentialPoints`, which returns the stored value.
